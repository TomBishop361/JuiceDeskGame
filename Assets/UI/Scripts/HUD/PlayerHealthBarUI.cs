...
public class PlayerHealthBarUI : MonoBehaviour {
	...
	private void UpdateMainFill(float deltaTime) {
		...
		// Smooth only the visible HUD fill. The actual Health component value changes immediately.
	...
	private void StartDelayedDamageBar() {
		...
		// Keep the delayed bar at the previous visible value, then drain it after a short pause.
	...
	private void UpdateFillColour(float deltaTime) {
		...
		// At critical health, add a subtle pulse so the player notices danger without needing health text.
	...
	private Color EvaluateHealthColour(float healthPercent) {
		...
		// Prevent inverted thresholds from producing odd colour blending in the Inspector.
		if (critical > warning) {
			float temp = critical;
			critical = warning;
			warning = temp;
